...
export async function getRecoveryStrategy(
  ...
  if (
    ...
    (message.includes("corrupted") || message.includes("parse"))
  ) {
    return {
      canRecover: true,
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries - 1) {
        console.warn(
          `${context} failed (attempt ${attempt + 1}/${maxRetries}), retrying...`,
        );
 ...
 */
export async function getErrorCategory(
  errorMessage: string,
): Promise<ErrorCategory> {
  try {
    const category = await invoke<string>("get_error_category", {
