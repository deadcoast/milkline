...
export function syncFarmerWithPlayer(currentPlayerState: PlayerState) {
    ...
    const trackChanged = previousPlayerState !== null &&
        previousPlayerState.currentTrack?.id !== currentPlayerState.currentTrack?.id;
    ...
    if (trackChanged && currentPlayerState.currentTrack && currentPlayerState.isPlaying) {
