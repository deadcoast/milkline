...
function createConfigStore() {
  ...
  return {
    ...
    setLibraryPath: (path: string | null) =>
      update((state) => ({ ...state, libraryPath: path })),
    setLastSkin: (skin: string | null) =>
      update((state) => ({ ...state, lastSkin: skin })),
    setVolume: (volume: number) => update((state) => ({ ...state, volume })),
    setVisualizerStyle: (style: "bars" | "waveform" | "spectrum") =>
      update((state) => ({ ...state, visualizerStyle: style })),
    setSpotifyEnabled: (enabled: boolean) =>
      update((state) => ({ ...state, spotifyEnabled: enabled })),
    setYoutubeEnabled: (enabled: boolean) =>
      update((state) => ({ ...state, youtubeEnabled: enabled })),
    setWindowPosition: (position: { x: number; y: number }) =>
      update((state) => ({ ...state, windowPosition: position })),
    setWindowSize: (size: { width: number; height: number }) =>
      update((state) => ({ ...state, windowSize: size })),
