...
describe('CropOverlay Component - Unit Tests', () => {
    ...
    it('should create a crop rectangle when user drags mouse', async () => {
        ...
        await fireEvent.mouseDown(overlay, { clientX: rect.left + 50, clientY: rect.top + 50 });
        await fireEvent.mouseMove(overlay, { clientX: rect.left + 200, clientY: rect.top + 150 });
        await fireEvent.mouseUp(overlay, { clientX: rect.left + 200, clientY: rect.top + 150 });
    ...
    it('should persist crop rectangle after mouse release', async () => {
        ...
        // Simulate mouse drag
        await fireEvent.mouseDown(overlay, { clientX: rect.left + 100, clientY: rect.top + 100 });
        await fireEvent.mouseMove(overlay, { clientX: rect.left + 300, clientY: rect.top + 250 });
        await fireEvent.mouseUp(overlay, { clientX: rect.left + 300, clientY: rect.top + 250 });
    ...
    it('should normalize rectangle when drawn in reverse direction', async () => {
        ...
        await fireEvent.mouseDown(overlay, { clientX: rect.left + 200, clientY: rect.top + 150 });
        await fireEvent.mouseMove(overlay, { clientX: rect.left + 50, clientY: rect.top + 50 });
        await fireEvent.mouseUp(overlay, { clientX: rect.left + 50, clientY: rect.top + 50 });
    ...
    it('should clamp coordinates to preview bounds', async () => {
        ...
        await fireEvent.mouseDown(overlay, { clientX: rect.left + 50, clientY: rect.top + 50 });
        await fireEvent.mouseMove(overlay, { clientX: rect.left + 10000, clientY: rect.top + 10000 }); // Way beyond bounds
        await fireEvent.mouseUp(overlay, { clientX: rect.left + 10000, clientY: rect.top + 10000 });
        ...
        expect(emittedCrop!.x + emittedCrop!.width).toBeLessThanOrEqual(defaultProps.sourceWidth);
        expect(emittedCrop!.y + emittedCrop!.height).toBeLessThanOrEqual(defaultProps.sourceHeight);
    });
    ...
    it('should clear previous crop when starting new drag', async () => {
        ...
        // Create first crop
        await fireEvent.mouseDown(overlay, { clientX: rect.left + 50, clientY: rect.top + 50 });
        await fireEvent.mouseMove(overlay, { clientX: rect.left + 150, clientY: rect.top + 150 });
        await fireEvent.mouseUp(overlay, { clientX: rect.left + 150, clientY: rect.top + 150 });
        ...
        await fireEvent.mouseDown(overlay, { clientX: rect.left + 200, clientY: rect.top + 200 });

        // Complete second crop
        await fireEvent.mouseMove(overlay, { clientX: rect.left + 400, clientY: rect.top + 300 });
        await fireEvent.mouseUp(overlay, { clientX: rect.left + 400, clientY: rect.top + 300 });
    ...
    it('should not emit cropChange for zero-size rectangles', async () => {
        ...
        await fireEvent.mouseDown(overlay, { clientX: rect.left + 100, clientY: rect.top + 100 });
        await fireEvent.mouseUp(overlay, { clientX: rect.left + 100, clientY: rect.top + 100 });
        ...
        await fc.assert(
            fc.asyncProperty(
                ...
                async ({ x1, y1, x2, y2 }) => {
                    ...
                    const overlay1 = container1.querySelector('.crop-overlay') as HTMLElement;
                    const rect1 = overlay1.getBoundingClientRect();

                    await fireEvent.mouseDown(overlay1, { clientX: rect1.left + x1, clientY: rect1.top + y1 });
                    await fireEvent.mouseMove(overlay1, { clientX: rect1.left + x2, clientY: rect1.top + y2 });
                    await fireEvent.mouseUp(overlay1, { clientX: rect1.left + x2, clientY: rect1.top + y2 });
                    ...
                    const overlay2 = container2.querySelector('.crop-overlay') as HTMLElement;
                    const rect2 = overlay2.getBoundingClientRect();

                    await fireEvent.mouseDown(overlay2, { clientX: rect2.left + x2, clientY: rect2.top + y2 });
                    await fireEvent.mouseMove(overlay2, { clientX: rect2.left + x1, clientY: rect2.top + y1 });
                    await fireEvent.mouseUp(overlay2, { clientX: rect2.left + x1, clientY: rect2.top + y1 });
