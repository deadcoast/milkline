...
use milk_lib::media_editor::{
    ...
    video_ops::{
        probe_video_metadata, probe_video_metadata_command, trim_and_crop_video,
...
fn create_test_video(path: &str, duration_sec: f64, width: u32, height: u32) -> Result<(), String> {
    let output = Command::new("ffmpeg")
        .args([
            ...
            "yuv420p",
            "-g",
            "1", // Keyframe interval for accurate seeking
            path,
        ])
        .output()
...
fn test_image_workflow_load_crop_export_verify() {
    ...
    assert_eq!(
        output_img.width(),
        200,
        "Output width should match crop width"
    );
    assert_eq!(
        output_img.height(),
        150,
        "Output height should match crop height"
    );
...
fn test_error_handling_invalid_trim_times() {
    ...
    assert!(
        result.is_ok(),
        "FFmpeg handles out-of-range times gracefully"
    );
}
