...
impl SystemAudioCapture {
    ...
    pub fn start(&mut self, app_handle: AppHandle) -> std::result::Result<(), MilkError> {
        ...
        let device = {
            // On other platforms, fall back to default input device
            host.default_input_device()
                .ok_or_else(|| MilkError::SystemAudio("No input device found".to_string()))?
        };
...
pub async fn start_system_audio_capture(
    app_handle: AppHandle,
    state: tauri::State<'_, Arc<Mutex<SystemAudioCapture>>>,
) -> std::result::Result<(), String> {
    let mut capture = state.inner().lock().unwrap();
...
pub async fn is_system_audio_capture_active(
    ...
    let capture = state.inner().lock().unwrap();
